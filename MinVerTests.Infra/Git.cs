...
namespace MinVerTests.Infra
{
    public static class Git
    {
        ...
        public static async Task<string> GetGraph(string path) =>
            (await CommandEx.ReadLoggedAsync("git", "log --graph --pretty=format:'%d'", path).ConfigureAwait(false)).Out;
        ...
        public static async Task<IEnumerable<string>> GetCommitShas(string path) =>
            (await CommandEx.ReadLoggedAsync("git", "log --pretty=format:\"%H\"", path).ConfigureAwait(false)).Out.Split('\r', '\n');
